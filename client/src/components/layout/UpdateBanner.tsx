...
export function UpdateBanner() {
  ...
  return (
    <div
      className="fixed z-50 bottom-24 md:bottom-6 left-1/2 -translate-x-1/2 w-[min(92vw,440px)] animate-in"
      role="status"
      aria-live="polite"
    >
      <div
        className={`relative rounded-2xl border shadow-2xl backdrop-blur-xl px-4 py-3 flex items-center gap-3 ${
          darkMode
            ? 'bg-gradient-to-r from-amber-600/95 to-orange-700/95 border-amber-500/40 text-slate-950 shadow-amber-900/40'
            : 'bg-gradient-to-r from-amber-500 to-orange-600 border-amber-600/40 text-slate-950 shadow-amber-500/30'
        }`}
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] font-semibold opacity-90 truncate">
            Hay cambios nuevos. Actualizá para aplicar las mejoras.
          </p>
        </div>
        ...
        <button
          onClick={applyUpdate}
          disabled={applying}
          className="flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-slate-950 text-amber-400 text-xs font-black uppercase tracking-wider shadow-lg transition-all duration-200 hover:scale-105 active:scale-95 disabled:opacity-60 disabled:hover:scale-100 shrink-0"
        >
